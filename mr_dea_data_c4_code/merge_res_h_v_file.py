...
from Common import check_path, list_files_in_directory, move_file, read_csv_get_df, clear_path
from GlobalConfig import tmp_res_out_path


# def merge_res_file(in_out_path):
#     in_f_list = list_files_in_directory(tmp_res_out_path)
#
#     # 先把非横纵数据先清理出去
#     for in_i_f in in_f_list:
#         if 'UE_V' not in in_i_f and 'UE_H' not in in_i_f:
#             print('move file11: ', in_i_f)
#             move_file(in_i_f, in_out_path)
#         else:
#             if 'UE_V' in in_i_f:
#                 tmp_h_f = in_i_f.replace('纵', '横').replace('V', 'H')
#             else:
#                 tmp_h_f = in_i_f.replace('横', '纵').replace('H', 'V')
#
...
def merge_file_new_format(in_out_path, in_c_v, in_c_h):
    ...
    for in_i_f in in_f_list:
        if in_c_v not in in_i_f and in_c_h not in in_i_f:
            ...
            if in_c_v in in_i_f:
                tmp_h_f = in_i_f.replace('纵', '横').replace('V', 'H').replace('v.csv', 'h.csv')
            else:
                tmp_h_f = in_i_f.replace('横', '纵').replace('H', 'V').replace('h.csv', 'v.csv')
