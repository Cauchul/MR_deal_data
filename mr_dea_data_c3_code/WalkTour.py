...
if __name__ == '__main__':
    ...
    all_data_path = r'D:\working\1206_2934国际财经中心测试\2934\NR\H1'
    ...
    for data_path in data_path_list:
        ...
        ue_df['ts'] = DataPreprocessing.convert_datetime_to_timestamp(ue_df['PC Time'])
        zcy_wifi_merger_df['ts'] = DataPreprocessing.convert_datetime_to_timestamp(zcy_wifi_merger_df['test_time'])
        df_data = pd.merge(ue_df, zcy_wifi_merger_df)
        ...
        if 'LTE' == net_type:
            ...
            res_df_data = DealData.deal_WalkTour_indoor_4g(df_data, net_type)
            df_write_to_csv(res_df_data, out_file)
        elif 'NR' == net_type:
            untreated_file = os.path.join(data_path, '原始_merge_文件.csv')
            df_write_to_csv(df_data, untreated_file)
            nr_res_df = DealData.deal_WalkTour_indoor_5g(df_data, net_type)
