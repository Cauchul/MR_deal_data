...
def set_config_info(in_config, in_data_type, in_section_name='WalkTour'):
    ...
    if 'uemr' == in_data_type:
        in_config.set(in_section_name, 'zcy_chart_file', '')


def set_UEMR_config_file_info(in_folder_path, in_data_type):
    in_section_name = 'DealUEMR'
    ...
    for i_path in res_list:
        ...
        for i_f in res_csv_file_list:
            if 'char' in i_f:
                ...
                in_config.set(in_section_name, 'uemr_src_file', i_f)
                print_with_line_number(f'ue log 文件： {i_f}', __file__)
...
if __name__ == '__main__':
    ...
    # 生成config文件
    set_UEMR_config_file_info(folder_path, 'finger')
    set_UEMR_config_file_info(folder_path, 'uemr')
