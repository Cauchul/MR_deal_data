...
from Common import copy_file, df_write_to_csv, find_nth_occurrence, merge_csv_file, list_files_in_directory
...
def merge_data(in_list, in_out_path):
    ...
    for i_file in in_list:
        ...
        if os.path.exists(tmp_file) and tmp_file != i_file:
            ...
            merge_df = merge_csv_file(i_file, tmp_file)
...
# 获取需要合并的文件list
def standard_out_file(in_out_path, in_clear_flag):
    ...
    for i_file in in_file_list:
        print(i_file)
        if 'LTE' in i_file or '4G' in i_file:
...
if __name__ == '__main__':
    ...
    standard_out_file(out_d_p, False)
