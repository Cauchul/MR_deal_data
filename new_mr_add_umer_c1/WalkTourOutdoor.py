...
class WalkTourOutdoor:
    def deal_WalkTour_4g(self, log_df_4g, in_out_standard_list):
        ...
        # 设置场景信息
        log_df_4g = wt_outdoor_set_scene_data(log_df_4g)
        # 时间转上海时区
    ...
    @staticmethod
    def deal_WalkTour_5g(log_df_5g, in_out_standard_list):
        # 删除测试log中 秒级重复数据，同秒取第一条
        ...
        # 设置场景信息
        log_df_5g = wt_outdoor_set_scene_data(log_df_5g)
        # 时间转上海时区
        ...
        log_df_5g = log_df_5g.reindex(columns=in_out_standard_list)
        # 获取领区数
...
def wt_outdoor_set_scene_data(log_df):
    # 设置场景信息
...
def standard_output_name(in_path, in_net_type, in_name_ue, in_name_d_time):
    cur_p_out_path = os.path.join(in_path, 'output')
    check_path(cur_p_out_path)
    ...
    tmp_cur_p_out_file = os.path.join(cur_p_out_path, file_name + '.csv')
    print('tmp_out_file: ', tmp_out_file)
    return tmp_out_file, tmp_cur_p_out_file


def process_one_piece_data(in_data_path):
    WTOutdoor = WalkTourOutdoor()

    # 获取ue、table等数据名称
    ue_file, table_file, zcy_file, wifi_bluetooth_file = WTOutdoor.get_data_file_path(in_data_path)

    # 获取所有数据的dataframe
    df_data = WTOutdoor.deal_ue_table_df(ue_file, table_file)
    df_data['ts'] = DataPreprocessing.convert_datetime_to_timestamp(df_data['PC Time'])

    # 获取数据是4G还是5G
    net_type = df_data['Network Type'][0]
    print('net_type: ', net_type)

    out_standard_list = WalkTour_table_format_dict[net_type]

    # 获取测试时间
    name_d_time = df_data['PC Time'][0].split(' ')[0]
    ...
    if '8539' in in_data_path:
        ...
        name_ue = 'UE'
        df_data['f_msisdn'] = f_msisdn_dict['2934']
    ...
    if 'LTE' == net_type:
        ...
        # 处理数据
        res_df_data = WTOutdoor.deal_WalkTour_4g(df_data, out_standard_list)
        ...
        df_write_to_csv(df_data, untreated_file)
        nr_res_df = WTOutdoor.deal_WalkTour_5g(df_data, out_standard_list)
...
if __name__ == '__main__':
    ...
    f_building = '国际财经中心'
    # 数据路径
    wt_outdoor_data_path = r'D:\working\1110室外\walktour\outdoor\LTE'
    # 处理数据
    process_one_piece_data(wt_outdoor_data_path)
