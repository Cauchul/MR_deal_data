...
class WalkTour:
    ...
    def indoor_get_df_wifi_bluetooth_zcy(in_config, in_deal_data):
        ...
        if isinstance(tmp_merger_df['test_time'][0], str):
            tmp_merger_df['ts'] = Time.convert_datetime_to_seconds(tmp_merger_df['test_time'])
        else:
            if 'nan' != str(tmp_merger_df['test_time'][0]):
                tmp_merger_df['ts'] = Time.convert_datetime_to_seconds(str(tmp_merger_df['test_time']))
            else:
                print_with_line_number(f"error, 走侧仪test_time转时间戳：{tmp_merger_df['test_time'][0]}，错误，详情请查看文件：{os.path.join(zcy_data_path, 'wifi_bluetooth_merge_data.csv')}")
                exit()
    ...
    def wifi_bluetooth_merge_ue_zcy_data(in_ue_df, in_zcy_df):
        ...
        # else:
        #     if 'nan' != str(in_zcy_df['test_time'][0]):
        #         in_zcy_df['ts'] = Time.convert_datetime_to_seconds(str(in_zcy_df['test_time']))
        #     else:
        #         print_with_line_number(f"error, 走侧仪当前时间戳：{in_zcy_df['test_time'][0]}，错误")
        #         exit()
        # in_zcy_df['ts'] = Common.change_time_zone(in_zcy_df['f_time'])

        # in_zcy_df['ts'] = in_zcy_df['created_by_ue_time']
        # in_zcy_df['ts'] = in_zcy_df['f_time']
        tmp_df = pd.merge(in_ue_df, in_zcy_df)
        if Common.check_df_empty(tmp_df):
            if not Common.check_valid_timestamp(in_zcy_df['ts'][0]):
                print_with_line_number(f"error, 走侧仪当前时间戳：{in_zcy_df['f_time'][0]}，无法转成datetime")
            print_with_line_number(f"error, test_log的时间戳为：{in_ue_df['ts'][0]}，走测仪wifi_bluetooth的时间戳为：{in_zcy_df['ts'][0]}，时间不一致，数据合并为空")
            exit()
    ...
    def merge_ue_zcy_data(in_ue_df, in_zcy_df):
        ...
        tmp_df = pd.merge(in_ue_df, in_zcy_df)
        if Common.check_df_empty(tmp_df):
            print_with_line_number(f"test_log的时间戳为：{in_ue_df['ts'][0]}，走测仪wifi_bluetooth的时间戳为：{in_zcy_df['ts'][0]}，时间不一致，数据合并为空")
            exit()
