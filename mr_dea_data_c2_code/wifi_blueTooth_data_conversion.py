...
if __name__ == '__main__':
    zcy_char_path = r'D:\working\1206_国际财经中心测试V1\场景2\8539\LTE\H1'
    # zcy_char_path = get_file_data()
    print('zcy_char_path: ', zcy_char_path)
    # zcy_char_file_name = 'F4-H17-4G-HUAWEI P40-4949_航站楼_4_4G_20231204采样点数据-chart.csv'
    # zcy_char_file_name = get_file_by_string('-chart', zcy_char_path)
    wifi_bt_file_name = get_file_by_string('_WiFi_BlueTooth', zcy_char_path)
    config_file = os.path.join(zcy_char_path, 'config.ini')

    # out_path = os.path.join(zcy_char_path, 'tmp_path')
    # check_path(out_path)
    # config_file = r'D:\12_1_data\11_30\zcy\A1\config.ini'
    generate_zcy_data()
    ...
    # print('len_north_y: ', len_north_y)
    # # 读取char数据
    # char_data = pd.read_csv(os.path.join(zcy_char_path, zcy_char_file_name))
    #
    # # 相对位置##x=10，y=47
    # new_x1_values = data_conversion(len_east_x, char_data['x'])
    # new_y1_values = data_conversion(len_north_y, char_data['y'])
    ...
    # lat = new_y1_values / 111000 + lat_O
    #
    # char_data['x_new'] = new_x1_values
    # char_data['y_new'] = new_y1_values
    # char_data['lon'] = lon
    # char_data['lat'] = lat
    #
    # generate_images()
