...
NR_table_data_columns = ['PCTime', 'NCI', 'IMEI', 'IMSI', 'NR gNodeB ID(24bit)']
LTE_table_data_columns = ['PCTime', 'ECI', 'IMEI', 'IMSI', 'eNodeB ID']

zcy_data_columns = ['f_time', 'f_x', 'f_y', 'f_longitude', 'f_latitude', 'f_direction', 'f_altitude']
...
def generate_images(in_x1_v, in_y1_v, in_lon, in_lat, in_out_path):
    ...
    png_file = os.path.join(in_out_path, 'xy_相对位置轨迹图.png')
    ...
    plt.tight_layout()
    # plt.show()
    png_file = os.path.join(in_out_path, 'lonlat_经纬度轨迹图.png')
    plt.savefig(png_file)
