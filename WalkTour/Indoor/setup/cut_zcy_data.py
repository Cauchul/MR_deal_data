...
if __name__ == '__main__':
    # 获取当前目录下的char文件
    data_path = r'E:\work\MR_Data\1月22号\20210122(1)_new_no_table\20210122\岳云伟\小米13\5G\2'
    ...
    for i_f in res_char_file:
        ...
        if 'clear' not in os.path.basename(i_f):
            ...
            # 多条件
            condition1 = (char_df['x'] < 979) & (char_df['y'] < 470) & (char_df['y'] > 200)
            # condition2 = (char_df['y'] > 250) & (char_df['y'] < 650)
            condition3 = (char_df['x'] < 988) & (char_df['y'] > 470)
            # filtered_df = char_df.loc[condition1]
            filtered_df = pd.concat([char_df.loc[condition1], char_df.loc[condition3]])
