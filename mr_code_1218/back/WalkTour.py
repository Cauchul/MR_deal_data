...
class WalkTour:
    ...
    def indoor_get_df_wifi_bluetooth_zcy(in_data_path):
        ...
        print(file_dict)
        # 获取zcy数据
        zcy_wifi_bluetooth_df = get_zcy_merge_wifi_bluetooth_data(file_dict['xyToLonLat_ZCY'],
                                                                  file_dict['xyToLonLat_WIFI_BlueTooth'])
        # 获取ue table数据
        ue_table_merge_df = WalkTour.deal_ue_table_df(file_dict['UE'], file_dict['table'])
...
if __name__ == '__main__':
    ...
    char_list = ['横', '纵']
    for i_c in char_list:
        for i in range(6):
            data_path = fr'D:\working\1214\test\国际财经中心\国际财经中心5G{i_c}{i}_20231214\walktour'

            # data_path = r'D:\working\1214\test\国际财经中心\国际财经中心5G纵1_20231214\walktour'
            # 获取所有带有table数据的目录
            data_path_list = get_all_data_path(data_path, 'table')  # outdoor
            for i_path in data_path_list:
                print('i_path: ', i_path)
                if 'S22' == os.path.basename(i_path).split('_')[1]:
                    f_device_brand = 'Samsung'
                else:
                    f_device_brand = 'iQOO'
                f_device_model = os.path.basename(i_path).split('_')[1]
