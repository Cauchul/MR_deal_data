...
class WalkTour:
    ...
    def indoor_get_df_wifi_bluetooth_zcy(in_config, in_deal_data):
        ...
        #     print_with_line_number(f"error, 走侧仪中的created_by_ue_time时间戳：{tmp_zcy_df['created_by_ue_time'][0]}，无法转成datetime")
        #     exit()
    ...
    def merge_ue_zcy_data(in_ue_df, in_zcy_df):
        ...
        if isinstance(in_zcy_df['test_time'][0], str):
            ...
            print_with_line_number('走测仪，char数据 created_by_ue_time 左合并到wifi蓝牙数据 f_time，合并后test_time列报错，详情请查看走测仪原始数据')
            if not Common.check_valid_timestamp(in_zcy_df['created_by_ue_time'][0]):
                print_with_line_number(f"error, 走测仪原始数据中的created_by_ue_time时间戳：{in_zcy_df['created_by_ue_time'][0]}，无法转成datetime")
            if not Common.check_valid_timestamp(in_zcy_df['f_time'][0]):
                print_with_line_number(f"error, 走测仪原始数据中的f_time时间戳：{in_zcy_df['f_time'][0]}，无法转成datetime")
