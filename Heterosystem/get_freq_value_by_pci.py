...
def get_freq_by_pci(in_data_file, in_pci):
    ...
    for i_pci_column in pci_columns:
        ...
        if np.isin(find_pci_arr, pci_data):
            ...
            # print(int(pci_to_freq_dict[in_pci]))
...
if __name__ == '__main__':
    ...
    # # 读取列名
    # df_columns = pd.read_csv(data_file, nrows=0)
    #
    # exchange_dict = {'f_pci': 'f_freq',
    #                  'f_pci_n1': 'f_freq_n1', 'f_pci_n2': 'f_freq_n2', 'f_pci_n3': 'f_freq_n3', 'f_pci_n4': 'f_freq_n4',
    #                  'f_pci_n5': 'f_freq_n5', 'f_pci_n6': 'f_freq_n6', 'f_pci_n7': 'f_freq_n7', 'f_pci_n8': 'f_freq_n8'}
    ...
    #
    # for i_pci_column in pci_columns:
    #     # pci_data = pd.read_csv(data_file, usecols=[i_pci_column])
    #     pci_data = get_df_by_column_list(data_file, [i_pci_column])
    #     # if np.isin(find_pci, data[i].values):
    #     #     print('成功')
    #
    #     find_pci_arr = np.array([find_pci])
    #     # 找到pci，然后通过pci找到频点
    #     if np.isin(find_pci_arr, pci_data):
    #         # print('成功')
    #         # 如果找到了pci，然后就根据pci找到
    #         all_df = get_df_by_column_list(data_file, [i_pci_column, exchange_dict[i_pci_column]])
    #         # print(all_df)
    #         pci_to_freq_dict = all_df.set_index(i_pci_column)[exchange_dict[i_pci_column]].to_dict()
    #
    #         print(int(pci_to_freq_dict[find_pci]))
    #         break
    #     else:
    #         print_with_line_number(f'未在 {i_pci_column} 列中找到 {find_pci}', __file__)
    # break
