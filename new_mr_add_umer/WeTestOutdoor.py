...
class WeTestOutdoor:
    def deal_wetest_4g(self, log_df_4g):
        ...
        # 设置场景信息
        log_df_4g = wetest_outdoor_set_scene_data(log_df_4g)
        # 时间转上海时区
    ...
    def deal_wetest_5g(self, log_df_5g):
        ...
        # 设置场景信息
        log_df_5g = wetest_outdoor_set_scene_data(log_df_5g)
        # 时间转上海时区
...
def wetest_outdoor_set_scene_data(log_df):
    # 设置场景信息
...
def process_one_piece_data(in_data_path, net_type):
    WeTestOut = WeTestOutdoor()
    # 获取ue、table等数据名称
    ue_file, table_file, zcy_file, wifi_bluetooth_file = WeTestOut.get_data_file_path(in_data_path)
    # 获取所有数据的dataframe
    df_data = WeTestOut.deal_ue_table_df(ue_file, table_file)
    df_data['ts'] = DataPreprocessing.convert_datetime_to_timestamp_v1(df_data['pc_time'])

    # 获取测试时间
    name_d_time = df_data['pc_time'][0].split(' ')[0]
    ...
    if 'LTE' == net_type:
        ...
        # 处理数据
        res_df_data = WeTestOut.deal_wetest_4g(df_data)
        ...
        df_write_to_csv(df_data, untreated_file)
        nr_res_df = WeTestOut.deal_wetest_5g(df_data)
...
if __name__ == '__main__':
    ...
    f_building = '国际财经中心'
    # 数据路径
    wetest_outdoor_data_path = r'E:\work\demo_merge\demo_test_data\wetest\outdoor\NR'
    # 设置输出路径
    out_data_path = r'E:\work\demo_merge\merged'
    clear_merge_path(out_data_path)
    check_path(out_data_path)
    # 处理数据
    process_one_piece_data(wetest_outdoor_data_path, 'NR')

    # 标准化输出文件的文件名
    standard_out_file(out_data_path, in_clear_flag=True)
