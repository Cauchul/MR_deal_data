...
class Config:
    ...
    def get_table_file(self):
        in_table_file = self.config.get(self.data_type, 'table_file')
        return in_table_file

    def get_char_file(self):
        in_table_file = self.config.get(self.data_type, 'char_file')
        return in_table_file

    def get_ue_file(self):
        in_table_file = self.config.get(self.data_type, 'ue_file')
        return in_table_file
...
class WeTest:
    ...
    def indoor_get_df(in_config, in_deal_zcy_char_csv_file):
        ...
        in_ue_df = Common.read_csv_get_df(in_ue_file)
        if '-' in in_ue_df['pc_time'][0] or '/' in in_ue_df['pc_time'][0]:
            # print('pc_time[1111]:', in_ue_df['pc_time'][0])
            res_zcy_ue_merge_df = WeTest.merge_ue_zcy_df(in_ue_df, in_zcy_df)
        else:
            # print('pc_time[2222]:', in_ue_df['pc_time'][0])
            res_zcy_ue_merge_df = WeTest.merge_ue_zcy_df_convert_format(in_ue_df, in_zcy_df)
...
class Common:
    ...
    def generate_output_file_name(in_data_path, in_df, in_net_type, in_n_scene, in_test_type):
        ...
        # print(district)
        if in_n_scene:
            tmp_out_file_name = f'{in_net_type}_{n_are}_{in_n_scene}_{in_test_type}_LOG_DT_UE_{name_d_time}.csv'
            # tmp_out_file_name = f'{in_net_type}_{n_are}_{in_n_scene}_{in_test_type}_LOG_DT_UE_{n_dev_id}_{name_d_time}'
        else:
            tmp_out_file_name = f'{in_net_type}_{n_are}_{in_test_type}_LOG_DT_UE_{name_d_time}.csv'
...
class DealData:
    ...
    def deal_walk_tour(self):
        ...
        # 保存一份处理之前的数据(添加接口)
        cur_path_out_file = os.path.join(cur_path_output, f'WT_{n_scene}_原始文件.csv')
    ...
    def deal_wetest(self):
        ...
        # 保存一份处理之前的数据(添加接口)
        cur_path_out_file = os.path.join(cur_path_output, f'WeTest_{n_scene}_原始文件.csv')
        print_with_line_number(f'原始df数据文件：{cur_path_out_file}')
        ...
        print_with_line_number(f'当前处理数据为：WeTest {net_type} {n_scene} 数据')
        print_with_line_number(f'输出结果文件为：{out_file}')
        Common.df_write_to_csv(res_ue_df, out_file)

    def deal_wifi_bluetooth(self):
        print('----开始处理walktour合并wifi、蓝牙数据-----')
        cur_path_output = os.path.join(os.path.dirname(self.config.get_ue_file()), 'output')
        Common.check_path(cur_path_output)
        ...
        # # 获取zcy，wifi和ue联合数据
        # in_ue_file = self.config.get_ue_file()
        # in_table_file = self.config.get_table_file()
        # in_char_file = self.config.get_char_file()
        # # 获取wifi 蓝牙文件
        # in_wifi_bluetooth_file = self.config.get_wifi_bluetooth_file()
        # # 处理char数据
        # tmp_zcy_df = self.deal_zcy_char_csv_file(in_char_file)
        # # 处理wifi 蓝牙
        # tmp_wifi_bluetooth_df = self.deal_wifi_bluetooth_csv_file(in_wifi_bluetooth_file)
        #
        # # 获取zcy，wifi 蓝牙合并数据
        # tmp_merger_df = pd.merge(tmp_wifi_bluetooth_df, tmp_zcy_df, left_on="f_time", right_on="created_by_ue_time",
        #                          how='left')
        ...
        # 保存一份处理之前的数据(添加接口)
        cur_path_out_file = os.path.join(cur_path_output, f'WT_wifi_bluetooth_{n_scene}_原始文件.csv')
        ...
        print(f'----WiFi Bluetooth {net_type} {n_scene} 数据-----')
        # 生成输出文件名称
...
if __name__ == '__main__':
    demo_config_file = r'E:\work\mr_dea_data_c2\mr_code_1216\demo_deal_config\demo_config.ini'
    config_project = Config(demo_config_file)
    # print(config_project.data_type)
    ...
    # char, table, UE
    # ue_file = config_project.get_ue_file()
    # table_file = config_project.get_table_file()
    # char_file = config_project.get_char_file()
