...
class WalkTour:
    ...
    def indoor_get_df_wifi_bluetooth_zcy(in_config, in_deal_data):
        ...
        print_with_line_number(f"走测仪原始数据保存为：{os.path.join(zcy_data_path, 'ddd_wifi_bluetooth_merge_data.csv')}")
        try:
            Common.df_write_to_csv(tmp_merger_df, os.path.join(zcy_data_path, 'ddd_wifi_bluetooth_merge_data.csv'))
        except PermissionError as e:
            print_with_line_number(f'写文件报错：{e}')

        # if Common.check_df_empty(tmp_merger_df['test_time'][0]):
        #     print_with_line_number(f"走测仪char数据和wifi_bluetooth数据合并后的数据test_time值为空")
        #     exit()
        # if not Common.check_valid_timestamp(tmp_wifi_bluetooth_df['f_time'][0]):
        #     print_with_line_number(f"error, wifi_bluetooth中的f_time时间戳：{tmp_wifi_bluetooth_df['f_time'][0]}，无法转成datetime")
        #     exit()
        # 获取ue table 合并数据
    ...
    def merge_ue_zcy_data(in_ue_df, in_zcy_df):
        ...
        if isinstance(in_zcy_df['test_time'][0], str):
            in_zcy_df['ts'] = Time.convert_datetime_to_seconds(in_zcy_df['test_time'])
            tmp_df = pd.merge(in_ue_df, in_zcy_df)
            return tmp_df
        else:
            print_with_line_number(
                '走测仪，char数据 created_by_ue_time 左合并到wifi蓝牙数据 f_time，合并后test_time列报错，详情请查看走测仪原始数据')
            if not Common.check_valid_timestamp(in_zcy_df['created_by_ue_time'][0]):
                print_with_line_number(
                    f"error, 走测仪原始数据中的created_by_ue_time时间戳：{in_zcy_df['created_by_ue_time'][0]}，无法转成datetime")
            if not Common.check_valid_timestamp(in_zcy_df['f_time'][0]):
                print_with_line_number(
                    f"error, 走测仪原始数据中的f_time时间戳：{in_zcy_df['f_time'][0]}，无法转成datetime")
            input('数据处理报错，输入任意键退出....')
            exit()
...
class DealData:
    ...
    def get_wifi_bluetooth_data(self):
        ...
        if not Common.check_valid_timestamp(tmp_zcy_df['created_by_ue_time'][0]):
            print_with_line_number(
                f"error, 走侧仪中的created_by_ue_time时间戳：{tmp_zcy_df['created_by_ue_time'][0]}，无法转成datetime")
        # 处理wifi 蓝牙
        ...
        if not Common.check_valid_timestamp(tmp_wifi_bluetooth_df['f_time'][0]):
            print_with_line_number(
                f"error, wifi_bluetooth中的f_time时间戳：{tmp_wifi_bluetooth_df['f_time'][0]}，无法转成datetime")
        # 获取zcy，wifi 蓝牙合并数据
        tmp_merger_df = pd.merge(tmp_wifi_bluetooth_df, tmp_zcy_df, left_on="f_time", right_on="created_by_ue_time",
                                 how='left')
