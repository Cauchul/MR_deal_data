...
def set_WeTest_config_file_info(in_folder_path, in_data_type):
    ...
    for i_path in res_list:
        ...
        for i_f in res_csv_file_list:
            if 'char' in i_f:
                res_config.set(in_section_name, 'zcy_chart_file', i_f)
                in_test_area = 'indoor'
                print_with_line_number(f'char文件： {i_f}', __file__)
            elif 'table' in i_f:
                ...
                res_config.set(in_section_name, '45g_test_log', i_f)
                print_with_line_number(f'ue log 文件： {i_f}', __file__)

        set_config_info(res_config, in_data_type, in_test_area)
        print('---' * 50)

        with open(os.path.join(i_path, f'{in_section_name}_{in_data_type}_config.ini'), 'w', encoding='UTF-8') as f:
            res_config.write(f)


if __name__ == '__main__':
    folder_path = r'E:\work\demo\20240116\4G\1'

    set_WeTest_config_file_info(folder_path, 'finger')
    set_WeTest_config_file_info(folder_path, 'uemr')
