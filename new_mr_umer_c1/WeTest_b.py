...
from Common import get_file_by_string, get_file_dict, get_zcy_data, read_csv_get_df, \
    ...
    generate_output_file_name, get_all_data_path, check_path, split_path_get_list, check_file_exists
...
def wetest_outdoor(in_data_path, in_set_scene_data):
    ...
    for i_data_f in file_list:
        in_res_df = wetest_outdoor_get_df(i_data_f)
        ...
        # out_file, cur_p_out_f = standard_output_name(data_path, net_type, 'ue', name_d_time)
...
def wetest_indoor(in_data_path, in_set_scene_data):
    ...
    n_scene = 'Indoor'
    print('室内')
    in_res_df = wetest_indoor_get_df(in_data_path)
    if 'lte_enb_id' in in_res_df.columns:
        print('4G')
        net_type = '4G'
        in_res_df = WeTest.deal_wetest_4g(in_res_df, in_set_scene_data)
...
def wetest(in_data_path, in_set_scene_data):
    ...
    # 获取测试时间
    # name_d_time = in_res_df['pc_time'][0].split(' ')[0]
    # name_d_time = name_d_time[name_d_time.find('/'):].replace('/', '')
    # print('name_d_time: ', name_d_time)
...
def wetest_indoor_get_df(in_data_path):
...
# 输入ue文件而不是路径
def wetest_outdoor_get_df(res_ue_file):
...
if __name__ == '__main__':
    ...
    indoor_data_path = r'E:\work\demo_merge\demo_test_data\wetest\indoor'
    # wetest(data_path, wetest_set_scene_data)

    data_path_list = get_all_data_path(indoor_data_path, 'UE_flag')  # outdoor
    ...
    outdoor_data_path = r'E:\work\demo_merge\demo_test_data\wetest\outdoor'
    outdoor_data_path_list = get_all_data_path(outdoor_data_path, 'UE_flag')
    outdoor_data_path_list = list(set(outdoor_data_path_list))
    # 去重
    print('outdoor_data_path_list: ', outdoor_data_path_list)
    ...
    # net_type = ''
    # res_df = None
    # if get_file_by_string('zip', data_path):
    #     print('室内')
    #     res_df = wetest_indoor(data_path)
    #     if 'lte_enb_id' in res_df.columns:
    #         print('4G')
    #         net_type = '4G'
    #         res_df = WeTest.deal_wetest_4g(res_df, wetest_set_scene_data)
    #     elif 'nr_gnb_id' in res_df.columns:
    #         print('5G')
    #         net_type = '5G'
    #         res_df = WeTest.deal_wetest_5g(res_df, wetest_set_scene_data)
    #     else:
    #         print(f'error, f_enb_id and f_gnb_id not find in res_df columns')
    # else:
    #     print('室外')
    ...
    #     for i_data_f in file_list:
    #         res_df = wetest_outdoor(i_data_f)
    #         if 'lte_enb_id' in res_df.columns:
    #             net_type = '4G'
    #             res_df = WeTest.deal_wetest_4g(res_df, wetest_set_scene_data)
    #         elif 'nr_gnb_id' in res_df.columns:
