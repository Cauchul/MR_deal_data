...
class WeTest:
    ...
    def merge_ue_zcy_df(in_ue_df, in_zcy_df):
        ...
        in_ue_df = pd.merge(in_ue_df, in_zcy_df, left_on="create_time", right_on="created_by_ue_time", how='right')
    ...
    # def merge_ue_zcy_df(in_ue_df, in_zcy_df):
    #     # in_ue_df = pd.merge(in_ue_df, in_zcy_df, left_on="pc_time", right_on="test_time", how='left')
    #     in_ue_df = pd.merge(in_ue_df, in_zcy_df, left_on="pc_time", right_on="test_time", how='right')
    ...
    def indoor_get_df(in_config, in_deal_zcy_char_csv_file):
        ...
        # 合并ue和char数据
        res_zcy_ue_merge_df = pd.merge(in_ue_df, in_zcy_df, left_on="create_time", right_on="created_by_ue_time", how='left')
    ...
    def deal_wetest_4g(log_df_4g):
        # 删除测试log中 秒级重复数据，同秒取第一条。
        Common.delete_df_duplicate_data_by_second(log_df_4g, log_df_4g['created_by_ue_time'])
...
class WalkTour:
    ...
    def merge_ue_zcy_data(in_ue_df, in_zcy_df):
        in_ue_df['ts'] = Time.convert_datetime_to_seconds(in_ue_df['PC Time'])
        print_with_line_number(f"test log PC Time为：{in_ue_df['ts'][0]}")
